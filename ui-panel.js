...
const alldbs = document.getElementsByName("alldbs")[0];
const alltblname = document.getElementsByName("alltblname")[0];
const alldbtblname = document.getElementsByName("alldbtblname")[0];
const allcolumns = document.getElementsByName("allcolumns")[0];
const dbcolumnname = document.getElementsByName("dbcolumnname")[0];
const dbdumptc = document.getElementsByName("dbdumptc")[0];
...
alldbs.addEventListener('click', anonClickMenuFunct, false);
alltblname.addEventListener('click', anonClickMenuFunct, false);
alldbtblname.addEventListener('click', anonClickMenuFunct, false);
allcolumns.addEventListener('click', anonClickMenuFunct, false);
dbdumptc.addEventListener('click', anonClickMenuFunct, false);
dbcolumnname.addEventListener('click', anonClickMenuFunct, false);
    ...
    switch (event.currentTarget.name) {
        ...
        case 'sltUnionnull':
            ...
            function multiString(text, count) {
                var ret = ""; for (var i = 0; i < count; i++) { ret += text; }
                return ret;
            };
        ...
        case 'datadump':
            ...
            break;
        case 'alldbs':
            newString = "schema_name :: From information_schema.schemata";
            this.setSelectedText(newString);
            break;
        case 'alltblname':
            newString = "table_name :: From information_schema.tables where table_schema='{dbname here}'";
            this.setSelectedText(newString);
            break;
        case 'alldbtblname':
            newString = "table_name :: From information_schema.tables";
            this.setSelectedText(newString);
            break;
        case 'allcolumns':
            newString = "column_name :: From information_schema.columns where table_name = '{table name here}'";
            this.setSelectedText(newString);
            break;
        case 'dbcolumnname':
            newString = "concat({column name here}, ':', {column name here}) :: From {table name here}";
            this.setSelectedText(newString);
            break;
        case 'dbdumptc':
            newString = "concat(table_name,':', column_name) :: From information_schema.columns";
            this.setSelectedText(newString);
            break;
        case 'sltDatadump':
...
function loadUrl() {
    ...
    if (webview) {
        let result = webview.getURL();
        // result = result.slice(0, -1);
        urlfield.value = result;
        console.log("Current WebView URL:", result);
    } else {
...
function execute() {
    ...
    var webview = document.querySelector("webview"); // Get WebView element
    ...
    if (typePostdata === "formdata") {
        ...
        webview.executeJavaScript(scriptpost);
    }
    else {
        var responsePost = "";
        fetch(url, {
